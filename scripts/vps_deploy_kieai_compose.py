...
services:
  kieai-proxy:
    ...
    networks:
      - n8n_default
    environment:
      - KIE_API_KEY=${KIE_API_KEY:?KIE_API_KEY must be set}
      - OMNIROUTE_API_KEY=${OMNIROUTE_API_KEY:?OMNIROUTE_API_KEY must be set}
      - KIEAI_PROXY_PORT=11434
    command: >
      /bin/bash -c "pip install flask requests -q &&
      cat > /app/proxy.py << 'PYEOF'
      ...
      def health():
          return jsonify({'status': 'ok', 'proxy': 'kieai-proxy-v7-streaming-responses'})

      if __name__ == '__main__':
          print(f"Kie.ai + OmniRoute Proxy v7 running on port {PORT}")
...
try:
    subprocess.run(scp_cmd(tmp_path, "/docker/kieai-proxy/docker-compose.yml"), check=True)
