            ...
            async with self._execution_lock:
                ...
                try:
                    ...
                    if signal.signal in ("BUY", "SELL") and signal.confidence >= min_confidence:
                        ...
                        # Pyramid only if price improves by % AND by ATR distance (compounds with confidence increase)
                        ...
                        if existing and last_layer_price is not None:
                            # ATR-based separation requirement (more robust than fixed %)
                            _atr_recent = (bars[-1].get("high", 0) - bars[-1].get("low", 0)) if isinstance(bars[-1], dict) else 0
                            _atr_separation_needed = _atr_recent * self._pyramid_atr_multiplier
                            _pct_improvement_needed = last_layer_price * self._pyramid_min_improvement
                            _required_separation = max(_atr_separation_needed, _pct_improvement_needed)
                            if existing.direction == "BUY" and current_price >= last_layer_price + _required_separation:
                                price_improved = True
                            elif existing.direction == "SELL" and current_price <= last_layer_price - _required_separation:
                                price_improved = True
                        ...
                        # CORRELATION CAP: compute current directional notional, block new entries that would exceed cap
                        try:
                            _all_open = db.query(Trade).filter(Trade.status.in_(["open", "filled"])).all()
                            _long_notional = sum(t.quantity * (t.entry_price or 0) for t in _all_open if t.direction == "BUY")
                            _short_notional = sum(t.quantity * (t.entry_price or 0) for t in _all_open if t.direction == "SELL")
                        except Exception:
                            _long_notional = _short_notional = 0.0
                        _new_notional = (float(os.getenv('TRADE_USDT_AMOUNT', '10')) * self._leverage) if hasattr(self, "_leverage") else float(os.getenv('TRADE_USDT_AMOUNT', '10')) * 10

                        if signal.signal == "BUY" and (not existing or can_add_layer):
                            # check LONG side exposure cap
                            if (not existing) and (_long_notional + _new_notional > self._max_directional_exposure_usdt):
                                logger.warning(f"  [ {symbol} ] BUY blocked: LONG exposure ${_long_notional:.0f}+${_new_notional:.0f} > cap ${self._max_directional_exposure_usdt:.0f}")
                                return
                            ...
                            # check SHORT side exposure cap
                            if (not existing) and (_short_notional + _new_notional > self._max_directional_exposure_usdt):
                                logger.warning(f"  [ {symbol} ] SELL blocked: SHORT exposure ${_short_notional:.0f}+${_new_notional:.0f} > cap ${self._max_directional_exposure_usdt:.0f}")
                                return
                            if not existing and self._open_count >= max_positions:
