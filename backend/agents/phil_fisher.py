...
def phil_fisher_agent(state: AgentState, agent_id: str = "phil_fisher_agent"):
    ...
    for ticker in tickers:
        ...
        progress.update_status(
            agent_id, ticker, "Analyzing management efficiency & leverage"
        )
        ...
        progress.update_status(
            agent_id, ticker, "Generating Phil Fisher-style analysis"
        )
        ...
        progress.update_status(
            agent_id, ticker, "Done", analysis=fisher_output.reasoning
        )
    ...
    if len(revenues) >= 2:
        ...
        if oldest_rev > 0 and latest_rev > 0:
            ...
            if rev_growth > 0.20:  # 20% annualized
                raw_score += 3
                details.append(
                    f"Very strong annualized revenue growth: {rev_growth:.1%}"
                )
                ...
                details.append(
                    f"Minimal or negative annualized revenue growth: {rev_growth:.1%}"
                )
        else:
    ...
    if len(eps_values) >= 2:
        ...
        if oldest_eps > 0 and latest_eps > 0:
            ...
            if eps_growth > 0.20:  # 20% annualized
                ...
                details.append(
                    f"Minimal or negative annualized EPS growth: {eps_growth:.1%}"
                )
        else:
